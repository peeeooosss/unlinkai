...
import {
  UserPlus, Loader2, ChevronDown, ChevronUp, Plus, X, CheckCircle,
...
interface CreateStudentFlowProps {
  open?: boolean;
...
const emptyEducation: EducationEntry = {
  institution: "", country: "", degree: "", fieldOfStudy: "",
  startYear: "", endYear: "", gpa: "", gradingScale: "percentage", stillEnrolled: false,
};

const emptyEnglishTest: EnglishTestEntry = {
  testType: "IELTS", overallScore: "", listening: "", reading: "",
  writing: "", speaking: "", testDate: "", trfNumber: "",
};

const emptyWorkEntry: WorkEntry = {
  employer: "", jobTitle: "", industry: "", startDate: "",
  endDate: "", isCurrent: false, employmentType: "full-time", description: "",
};

export function CreateStudentFlow({ open: controlledOpen, onOpenChange: controlledOnOpenChange, trigger, onStudentCreated }: CreateStudentFlowProps) {
  const [internalOpen, setInternalOpen] = React.useState(false);
  const open = controlledOpen !== undefined ? controlledOpen : internalOpen;
  const onOpenChange = controlledOnOpenChange || setInternalOpen;

  const [loading, setLoading] = React.useState(false);
  const [step, setStep] = React.useState<"form" | "document">("form");
  ...
  // Education
  const [educationHistory, setEducationHistory] = React.useState<EducationEntry[]>([{ ...emptyEducation }]);

  // English Tests
  const [englishTests, setEnglishTests] = React.useState<EnglishTestEntry[]>([{ ...emptyEnglishTest }]);
  ...
  const [hasWorkExperience, setHasWorkExperience] = React.useState(false);

  // Financial
  const [budget, setBudget] = React.useState("");
  const [budgetCurrency, setBudgetCurrency] = React.useState("GBP");
  const [fundingSource, setFundingSource] = React.useState("");
  const [bankStatementAvailable, setBankStatementAvailable] = React.useState(false);

  // Visa History
  const [hasVisaRefusals, setHasVisaRefusals] = React.useState(false);
  const [visaRefusalDetails, setVisaRefusalDetails] = React.useState("");
  const [hasStudiedAbroad, setHasStudiedAbroad] = React.useState(false);
  const [studyAbroadDetails, setStudyAbroadDetails] = React.useState("");
  const [currentVisaStatus, setCurrentVisaStatus] = React.useState("");

  // Preferences
  const [countryPreferences, setCountryPreferences] = React.useState<string[]>([]);
  const [coursePreferences, setCoursePreferences] = React.useState<string[]>([]);
  const [studyLevel, setStudyLevel] = React.useState("Postgraduate");
  const [preferredInstitutions, setPreferredInstitutions] = React.useState("");
  const [intakePreference, setIntakePreference] = React.useState("");
  const [accommodationPref, setAccommodationPref] = React.useState("");
  const [postStudyWork, setPostStudyWork] = React.useState("");
  ...
  const addEnglishTest = () => setEnglishTests((prev) => [...prev, { ...emptyEnglishTest }]);
  ...
  // --- Work helpers ---
  const addWork = () => setWorkExperience((prev) => [...prev, { ...emptyWorkEntry }]);
  ...
  const toggleCountry = (c: string) => setCountryPreferences((prev) =>
    prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c]
  );
  const toggleCourse = (c: string) => setCoursePreferences((prev) =>
    prev.includes(c) ? prev.filter((x) => x !== c) : [...prev, c]
  );

  const resetAll = () => {
    setTitle(""); setName(""); setEmail(""); setPhone(""); setWhatsapp("");
    setNationality(""); setCountryOfResidence(""); setPassportNumber("");
    setPassportExpiry(""); setDob(""); setGender(""); setAddress(""); setCity("");
    setNativeLanguage(""); setEmergencyName(""); setEmergencyPhone("");
    setEducationHistory([{ ...emptyEducation }]);
    setEnglishTests([{ ...emptyEnglishTest }]);
    setStandardizedTests([]);
    setShowStandardized(false);
    setWorkExperience([]);
    setHasWorkExperience(false);
    setBudget(""); setBudgetCurrency("GBP"); setFundingSource("");
    setBankStatementAvailable(false);
    setHasVisaRefusals(false); setVisaRefusalDetails("");
    setHasStudiedAbroad(false); setStudyAbroadDetails(""); setCurrentVisaStatus("");
    setCountryPreferences([]); setCoursePreferences([]);
    setStudyLevel("Postgraduate"); setPreferredInstitutions("");
    setIntakePreference(""); setAccommodationPref(""); setPostStudyWork("");
    setAdditionalNotes(""); setConsentGiven(false);
    setError("");
  };
  ...
  const handleDraftSubmit = async () => {
    if (!name || !email || !phone) {
      setError("Name, Email, and Phone are required for a draft.");
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!consentGiven) {
      setError("Consent is required to create a student profile.");
      return;
    }
        ...
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto bg-white border-black p-0">
          <DialogHeader className="sticky top-0 z-10 bg-white border-b border-black px-6 py-4">
            <DialogTitle className="flex items-center gap-2 text-neutral-900 text-xl">
              ...
              {step === "form" ? "Add New Student" : "Add Documents"}
            </DialogTitle>
            {step === "form" && (
              <p className="text-sm text-neutral-600">
                Complete student profile for study abroad applications. Fields marked with * are required.
              </p>
            )}
          </DialogHeader>
          ...
          {step === "form" ? (
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <Section title="Personal Details" icon={UserPlus} defaultOpen>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <Field label="Full Name *" id="name" placeholder="e.g. Priya Sharma" value={name} onChange={setName} required />
                  <Field label="Email *" id="email" type="email" placeholder="email@example.com" value={email} onChange={setEmail} required />
                  <Field label="Phone *" id="phone" placeholder="+91 98765 43210" value={phone} onChange={setPhone} required />
                  <Field label="WhatsApp" id="whatsapp" placeholder="Separate if different from phone" value={whatsapp} onChange={setWhatsapp} />
                  <Field label="Nationality *" id="nationality" placeholder="e.g. Indian" value={nationality} onChange={setNationality} required />
              ...
              <Section title="Education History" icon={GraduationCap}>
                <p className="text-xs text-neutral-500 mb-3">Add all qualifications from high school onwards. This is required for university applications.</p>
                {educationHistory.map((entry, i) => (
                  <div key={i} className="border border-neutral-200 rounded-lg p-4 space-y-3 mb-3 bg-neutral-50">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Institution Name *" id={`edu-inst-${i}`} placeholder="e.g. Delhi University" value={entry.institution} onChange={(v) => updateEducation(i, "institution", v)} required />
                ...
                {englishTests.map((entry, i) => (
                  <div key={i} className="border border-neutral-200 rounded-lg p-4 space-y-3 mb-3 bg-neutral-50">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <Field label="Overall Score *" id={`eng-overall-${i}`} placeholder={entry.testType === "IELTS" ? "e.g. 7.0" : entry.testType === "TOEFL" ? "e.g. 100" : "e.g. 65"} value={entry.overallScore} onChange={(v) => updateEnglishTest(i, "overallScore", v)} required />
              ...
              <div className="flex justify-end gap-3 pt-2 border-t border-black">
                ...
                <Button type="button" variant="outline" onClick={handleDraftSubmit} className="text-amber-700 border-amber-300 hover:bg-amber-50" disabled={loading}>
                  Save Draft
                </Button>
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white" disabled={loading}>
                  {loading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Creating...</> : "Create Student Profile"}
                </Button>
              </div>
            </form>
          ) : (
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 border border-green-200">
                ...
                <div>
                  <p className="text-sm font-medium text-green-800">Student profile created successfully!</p>
                  <p className="text-xs text-green-600 mt-0.5">You can now add documents or close this dialog.</p>
                </div>
              </div>
              <Button variant="outline" className="w-full border-dashed border-green-300 text-green-700 hover:bg-green-50" onClick={() => setDocModalOpen(true)}>
